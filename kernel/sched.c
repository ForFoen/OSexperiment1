...
struct linux_dirent {
	...
	unsigned short d_reclen; //length of this dirent，4个字节
	char           d_name[14]; //filename，14个字节
};

int sys_getdents(unsigned int fd, struct linux_dirent *dirp, unsigned int count){
	int i,charnum,j,dirnum;
	char *c,buff[1024],*d;
	c=buff;
	dirnum=0;
	charnum=sys_read(fd,buff,count);
	for(i=0;i<charnum;i++){
	if(buff[i]!='\0')
		printk("%c",buff[i]);
	else
	...
	for(i=2;dirnum<charnum;i=i+16){
		dirp->d_ino=*((short int*)c);
		dirp->d_off=sizeof(struct linux_dirent); 
		dirp->d_reclen=sizeof(struct linux_dirent); 
		c+=2;
		dirnum+=dirp->d_reclen;
		for(j=0;j<14;j++)
			put_fs_byte(*c++,&(dirp->d_name[j]));
		d=(char*)dirp;
		d+=dirp->d_off;
		dirp=(struct linux_dirent*)d;
	}
	return dirnum;
//	int i,charnum,j,dirnum;
//	int buff[1024];
